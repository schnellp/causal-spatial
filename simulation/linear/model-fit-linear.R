...
fit.affine <- function(y, z, Xz, Xy,
  ...
  if (is.null(init[["beta"]])) {
    beta <- coefficients(glm(y ~ 0 + Xy, family = "poisson"))
  } else {
    beta <- init[["beta"]]
  }
  ...
  log.acs.test <- outer(drop(log.acs.prog), beta[z.coord] * z.test, `+`)
  ...
  for (iter in 2 : total.iters) {
    if (iter %% print.iters == 0) {
      print(paste(iter,
                  ...
                  ifelse(iter > burn.in + 1,
                         round(mean(trace[(burn.in + 1) : (iter - 1), "beta.z"]), 2),
                         NA),
                  ifelse(iter > burn.in + 1,
                         round(quantile(trace[(burn.in + 1) : (iter - 1), "beta.z"],
                                        prob = 0.025), 2),
                         NA),
                  ifelse(iter > burn.in + 1,
                         round(quantile(trace[(burn.in + 1) : (iter - 1), "beta.z"],
    ...
    fit <- glm(y ~ 0 + Xy + offset(u), family = "poisson")
    ...
    V.beta.lik.inv <- ginv(V.beta.lik)
    ...
    beta.pro <- as.vector(t(chol(post.par.approx$V)) %*% rnorm(py) +
                            post.par.approx$mu)
    post.mu <- post.par.approx$mu
    post.Prec <- solve(post.par.approx$V)
    ...
    z.coord <- which(colnames(Xy) == "z")
    log.acs.prog <- Xy[, -z.coord] %*% beta[-z.coord] + u
    log.acs <- outer(drop(log.acs.prog), beta[z.coord] * z.test, `+`)
    log.acs.offset <- max(log.acs)
    acs <- exp(log.acs.offset) * colMeans(exp(log.acs - log.acs.offset))
