...
fit.affine <- function(y, z, Xz, Xy,
  ...
  if (is.null(init[["beta"]])) {
    beta <- coefficients(glm(y ~ 0 + offset(offset) + Xy, family = "poisson"))
    for (i in missing.y) {
  ...
  z.coord <- which(colnames(Xy) == "z")
  log.acs.prog <- Xy[, -z.coord] %*% beta[-z.coord] + u
  log.acs.offset <- max(log.acs.prog)
  acs.scalar <- exp(log.acs.offset) * colMeans(exp(log.acs.prog - log.acs.offset))
  ...
  for (iter in 2 : total.iters) {
    if (iter %% print.iters == 0) {
      print(paste(iter,
                  ...
                  ifelse(iter > burn.in + 1,
                         ...
                         NA),
                  ifelse(iter > burn.in + 1,
                         round(100 * mean(trace[(burn.in + 1) : (iter - 1), "beta.z"]), 2),
                         NA),
                  ifelse(iter > burn.in + 1,
                         round(100 * quantile(trace[(burn.in + 1) : (iter - 1), "beta.z"],
                                        prob = 0.025), 2),
                         NA),
                  ifelse(iter > burn.in + 1,
                         round(100 * quantile(trace[(burn.in + 1) : (iter - 1), "beta.z"],
    ...
    fit <- glm(y ~ 0 + Xy + offset(offset + u), family = "poisson")
    
    if (!fit$converged) {
      print(summary(fit))
      print(u)
      print(trace[(iter - 5) : (iter - 1), ])
      stop()
    }
    
    mu.beta.lik <- coefficients(fit)
    V.beta.lik <- vcov(fit)
    if (any(is.na(V.beta.lik)) || any(abs(V.beta.lik) == Inf)) {
      print(mu.beta.lik)
      print(V.beta.lik)
      plot(trace[, "beta.z"])
      print(u)
    }
    V.beta.lik.inv <- solve(V.beta.lik)

    mu.beta.prior <- rep(0, py)
    V.beta.prior <- diag(1 / coeff.prior, py)

    post.par.approx <- post.par.gaussian(prior.mu = mu.beta.prior,
                                         prior.V = V.beta.prior,
                                         lik.mu = mu.beta.lik,
                                         lik.V = V.beta.lik)

    beta.pro <- as.vector(t(chol(post.par.approx$V)) %*% rnorm(py) +
                            post.par.approx$mu)
    ...
    if (FALSE) {
      mh.accept.log.prob(
        ...
        log.prop.cur = - t(beta - post.par.approx$mu) %*% solve(post.par.approx$V) %*% (beta - post.par.approx$mu) / 2,
        log.prop.pro = - t(beta.pro - post.par.approx$mu) %*% solve(post.par.approx$V) %*% (beta.pro - post.par.approx$mu) / 2
      )
    }
